        ...
        <table id="basic-datatable" class="table dt-responsive">
          ...
          <tbody>
            ...
            <tr id="{{$issue->id}}">
              ...
              <td>
                <button type="button" class="btn btn-success btn-sm" onclick="show_edit_modal({{$issue}})"><i
                    class="la la-pencil"></i></button>

                <button type="button" class="btn btn-danger btn-sm" onclick="delete_issues({{$issue->id}})"><i
      ...
      <div class="card-body">
        ...
        <form id="addissue_form">
          ...
          <div class="form-group">
            ...
            <textarea class="form-control" rows="3" id="name" name="name" placeholder="Enter Description"
              required></textarea>
...
<script type="text/javascript">
  function addRow(data) {
    ...
    function show_edit_modal(issue){
        eliminate_errors('#issue_name','#issue_name_error');
        document.getElementById('issue_form').reset();
        $('#id').val(issue.id);
        $('#issue_name').val(issue.name);

        $('#editModal').modal('show');
    }

    $(document).ready(function(){
        $('#addissue_form').on('submit', function(evt) {
            evt.preventDefault();
            var self = this;
            $.ajax({
